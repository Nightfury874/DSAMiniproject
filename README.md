...
To make a header file, we have to create one file with a name, and extension should be "".h"". In that function there will be no ``` main()``` function. In that file, we can put some variables, some functions etc.<p style = "color:blue"> <b>To use that header file, it should be present at the same directory, where the program is located.</b> Now using  ```#include``` we have to put the header file name. The name will be inside double quotes. Include syntax will be look like this.
